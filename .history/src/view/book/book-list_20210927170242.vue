        ...
        <div class="Table">
          ...
          <TableData :config="table_config">
            <template #action="slotData">
              <a-button class="Edit" size="small" @click="edit(slotData.data.record.id)">编辑</a-button>&emsp;
              <a-popconfirm title="此操作将永久删除该图书，是否继续？" ok-text="确定" cancel-text="取消" @confirm="deleteBooks(slotData.data.record.id)">
...
export default {
  ...
  setup() {
    ...
    const edit = (record) => {
      router.push({ name: "book-detail", query: { id: record } });
      // console.log(record);
    };
