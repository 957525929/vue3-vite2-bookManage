        ...
        <div class="Table">
          <a-table class="ant-table-striped" :pagination="false" size="middle" :columns="columns" :data-source="data" :rowClassName="(record, index) => (index % 2 === 1 ? 'table-striped' : null)">
            <template #action="{ record }">
              ...
              <a-popconfirm title="Are you sure delete this task?" ok-text="Yes" cancel-text="No" @confirm="deleteBooks(record)" @cancel="cancel">
                <a-button danger size="small" @click="deleteBooks(record)">删除</a-button>
              </a-popconfirm>
            </template>
          </a-table>
        </div>
