<template>
  <div>
    <div class="Title">
      <div>分组管理信息</div>
      <a-select v-model:value="searchData.group_id" label-in-value style="width: 120px" :options="options" placeholder="请选择分组"> </a-select>
    </div>
    <div class="Content">
      <div class="Table">
        <!-- <a-table class="ant-table-striped" :pagination="false" size="middle" :columns="columns" :data-source="data" rowKey="id" :rowClassName="(record, index) => (index % 2 === 1 ? 'table-striped' : null)">
          <template #action="{ record }">
            <a-button class="Edit" size="small" @click="showInfo(record)">信息</a-button>
            &emsp;
            <a-button size="small" @click="edit(record)">权限</a-button>
            &emsp;
            <a-popconfirm title="此操作将永久删除该分组，是否继续？" ok-text="确定" cancel-text="取消" @confirm="deleteGroup(record)">
        ...
        <TableData :config="table_config">
          <template #name="slotData">
            <span v-for="item in slotData.data.record.groups" :key="item.id">
              {{ item.name + " " }}
            </span>
          </template>
          <template #action="slotData">
            <a-button class="Edit" size="small" @click="showModal(slotData.data.record)">编辑</a-button>&emsp;
            <a-popconfirm title="此操作将永久删除该用户，是否继续？" ok-text="确定" cancel-text="取消" @confirm="deleteBooks(slotData.data.record)">
              <a-button danger size="small">删除</a-button>
            </a-popconfirm>
            <a-button class="Edit" size="small" @click="showInfo(slotData.data.record)">信息</a-button>
            &emsp;
            <a-button size="small" @click="edit(slotData.data.record)">权限</a-button>
            &emsp;
            <a-popconfirm title="此操作将永久删除该分组，是否继续？" ok-text="确定" cancel-text="取消" @confirm="deleteGroup(slotData.data.record)">
              <a-button danger size="small">删除</a-button>
            </a-popconfirm>
          </template>
        </TableData>
    ...
    <!-- 信息modal -->
    <a-modal v-model:visible="visible" title="分组信息" @ok="handleOk" width="1000px" okText="确定" cancelText="重置">
...
//api
import { getUsersGroup, deletePermissions, updatePermissions } from "@/api/api.js";
export default {
  components: {
    TableData,
  },
