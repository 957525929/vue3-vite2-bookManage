  ...
  <div>
    <a-card>
      ...
      <a-modal v-model:visible="visible" title="用户信息" @ok="handleOk" width="1000px">
        <a-tabs v-model:activeKey="activeKey" @change=handler>
...
//api
import { getUsersList, deleteUser, updateUsers } from "@/api/api.js";
export default {
  setup() {
    const plainOptions = [
      {
    ...
    const handleOk = async () => {
      let res = await updateUsers(formState.id, formState);
      message.success(res.data.message);
      getList();
      visible.value = false;
    };
