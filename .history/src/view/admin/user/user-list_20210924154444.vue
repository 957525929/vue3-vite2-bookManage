<template>
  <div>
    <a-card>
      ...
      <a-modal v-model:visible="visible" title="用户信息" @ok="handleOk" width="1000px">
...
export default {
  setup() {
    ...
    const showModal = (record) => {
      ...
      // state.group_ids = record.groups.length == 2 ? record.groups[1].id : record.groups[0].id;
      // formState.group_ids = record.groups.length == 2 ? record.groups[1].id : record.groups[0].id;
      // console.log(record.groups.length == 2 ? record.groups[0].id + "," + record.groups[1].id : record.groups[0].id);
      formState.group_ids = record.groups.length == 2 ? [record.groups[1].id, record.groups[1].id] : [record.groups[0].id];
    };
