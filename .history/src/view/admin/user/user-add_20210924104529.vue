<template>
  <div>
    <a-card>添加用户</a-card>
    <a-card>
      <a-form name="custom-validation" ref="ruleForm" ref="formRef" :model="formState" :rules="rulesRef" :label-col="labelCol" :wrapper-col="wrapperCol">
...
export default {
  setup() {
    ...
    const checkPasswords = async (rule, value, callback) => {
      const pas = formState.confirm_password;
      if (!value) {
