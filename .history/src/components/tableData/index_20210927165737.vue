<template>
  <a-table class="ant-table-striped" :pagination="false" size="middle" :data-source="data" rowKey="id" :rowClassName="(record, index) => (index % 2 === 1 ? 'table-striped' : null)">
    <template v-for="item in columns">
      <a-table-column v-if="item.type === 'slot'" :title="item.title" :dataIndex="item.dataIndex" :key="item.dataIndex">
        <template #action="slotData">
          <slot :name="item.slots.customRender" :data="slotData.row"></slot>
        </template>
      </a-table-column>
      <a-table-column v-else :title="item.title" :dataIndex="item.dataIndex" :key="item.dataIndex"> </a-table-column>
    </template>
  </a-table>
</template>
