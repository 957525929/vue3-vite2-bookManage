<template>
  <a-table class="ant-table-striped" :pagination="false" size="middle" :data-source="data" rowKey="id" :rowClassName="(record, index) => (index % 2 === 1 ? 'table-striped' : null)">
    <template v-for="item in columns" :key="item.key">
      <a-table-column v-if="item.type === 'slot'" :title="item.title" :dataIndex="item.dataIndex" :dataIndex="item.dataIndex">
        <template>
          <slot name="action"></slot>
        </template>
      </a-table-column>
      <a-table-column v-else :title="item.title" :dataIndex="item.dataIndex"> </a-table-column>
    </template>
  </a-table>
</template>
